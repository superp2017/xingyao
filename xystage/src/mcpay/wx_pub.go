...
func wx_pub_pay(order *common.ST_Order) (map[string]string, error) {
	...
	Info("errCode = %s, msg = %s\n", ret["return_code"], ret["return_msg"])
...
// func wx_pub_transfers(transfer *JsOrder.ST_Transfer) (map[string]string, error) {

// 	g_user_transfer_mutex.Lock()

// 	cache, ok := g_user_transfer[transfer.OpenId]
// 	if !ok {
// 		tc := &transfer_cache{1, 0, nil}
// 		g_user_transfer[transfer.OpenId] = tc
// 	} else {

// 		if cache.N > C_SENDLIMIT {
// 			if cache.OrderList == nil {
// 				cache.OrderList = make([]*JsOrder.ST_Transfer, 0)
// 			}
// 			for _, v := range cache.OrderList {
// 				if transfer.OrderId == v.OrderId {
// 					g_user_transfer_mutex.Unlock()
// 					return nil, nil
// 				}
// 			}

// 			cache.N = cache.N + 1
// 			cache.OrderList = append(cache.OrderList, transfer)
// 			cache.HourAmount += transfer.Amount
// 			g_user_transfer[transfer.OpenId] = cache
// 			g_user_transfer_mutex.Unlock()
// 			return nil, nil
// 		}
// 	}
...
// 	return direct_transfer(transfer)
// }
